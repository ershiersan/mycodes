...
class codes
{
     ...
     * @return array
     */
    protected function getCodingMsgByIndex($index, $autoincreament=false) {
        /**
         * 如果$autoincreament为true，current的值加1保存
    ...
    private function callCoding($action='verify') {
        try{
            $arrReturn = $this->getCodingMsgByIndex($this->_index, $action=="create");
        } catch (Exception $e) {
            return false;
        }
    ...
    public function encode($index, $code_length=11) {
        ...
        $this->_index = $index;
        $indexMsg = $this->callCoding('create');
